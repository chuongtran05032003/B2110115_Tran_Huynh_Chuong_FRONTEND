<template>
  <form class="max-w-sm mx-auto mt-[30px]" @submit.prevent="handleSubmit(onSubmit())" :validation-schema="contactFormSchema">
    <div class="mb-5">
        ...
        <Field name="name" type="text" id="name"  v-model="contactLocal.name" required
    ...
    <div class="mb-5">
        ...
        <Field   name="email" type="email"  id="email"  v-model="contactLocal.email" placeholder="name@gmail.com"  required 
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
    ...
    <div class="mb-5">
        ...
        <Field   name="address" type="text"  id="address"  v-model="contactLocal.address" required 
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
    ...
    <div class="mb-5">
        ...
        <Field   name="phone"  type="tel"  id="phone"  v-model="contactLocal.phone" required 
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
        <ErrorMessage name="phone" class="text-red-600 text-sm mt-1" />
    </div>
    <div class="flex items-start mb-5">
        <div class="flex items-center h-5">
            <input id="favorite"  type="checkbox"  v-model="contactLocal.favorite"
    ...
    <div class="mb-5 flex justify-end space-x-2">
        <button type="button" @click="cancel" 
            class="bg-white border-solid border-[2px] text-gray-600 border-gray-600 hover:bg-gray-200 focus:ring-2 
                    focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2 text-center">
            <i class="fas fa-backward"></i> Thoát
        </button>
        <button v-if="this.contactLocal.name !==''" type="button" @click="deleteContact" 
            class="bg-white border-solid border-[2px] text-red-600 border-red-600 hover:bg-red-100 focus:ring-2 
                    focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2 text-center">
            <i class="fas fa-trash"></i> Xóa
        </button>
        <button type="submit" class="flex-1 bg-green-600 text-white hover:bg-green-700 focus:ring-2 focus:outline-none 
                                    focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2 text-center">
